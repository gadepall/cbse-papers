...
\begin{enumerate}
    ...
    \end{enumerate}
        \item For the following data, in Table 
			\label{table:prob-10-2}, draw a 'less than' ogive and hence find the median of the distribution.
		\begin{table}[!hb]
...
%			\scalebox{0.6}{
    ...
    \end{tabular}
	  ...
	  \ref{fig:prob-10-2.pdf}.  Let the age data be represented as
		...
		\end{align}
		If $n$ be the data size,  the median is obtained as 
		\begin{align}
			\vec{e}_1^{\top}\sbrak{\vec{A}_i + \frac{\frac{c_n}{2} -\vec{e}_2^{\top}  \vec{A}_i }{\vec{e}_2^{\top}\brak{\vec{A}_{i+1} -\vec{A}_i }}\brak{\vec{A}_{i+1} -\vec{A}_i }}
		\end{align}
		where
		\begin{align}
			\vec{A}_k = \myvec{a_k \\ c_k},
			c_i < \frac{c_n}{2} < c_{i+1}
		\end{align}
